...
def extension_package() -> dict:
    commands = [
        ...
        ("uacos.dashboard", "UACOS: Open Dashboard"),
...
def validate_vscode_pro_extension(output_dir: Path) -> dict:
    ...
    if (output_dir / "package.json").exists():
        try:
            ...
            for must in ["uacos.contextBudget", "uacos.autopilotPlan", "uacos.patch20Validate", "uacos.providerHealth", "uacos.dashboard"]:
                if must not in ids:
    ...
    if (output_dir / "extension.js").exists():
        ...
        for must in ["context-budget", "context-compressed", "autopilot-plan", "patch20-validate", "llm33-probe", "dashboard"]:
            if must not in text:
